 ...
 * ES Module - loads Transformers.js directly
 */
...
class RMBGProcessor {
    ...
    async loadModel() {
        ...
        try {
            ...
            this.updateModelStatus('Initializing AI model...', 5);
            ...
            this.updateModelStatus('Loading image processor...', 90);
            this.processor = await AutoProcessor.from_pretrained('Xenova/modnet');

            this.updateModelStatus('Model ready!', 100);
    ...
    async handleImageUpload(file) {
        try {
            ...
            if (file.size > maxSize) {
                this.showError('Image is too large. Please use an image smaller than 10MB.');
                return;
            }
    ...
    async processImage(img) {
        try {
            ...
            for (let i = 0; i < height * width; i++) {
                const alpha = Math.max(0, Math.min(255, maskData[i] * 255)); // Clamp to 0-255
